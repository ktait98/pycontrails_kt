...
if [ $loc == "NA" ]; then
   ...
   for datetime in "${datetimes[@]}"; do
      ...
      # Calculate t0_fl (one hour later)
      t0_fl=$(date -d "$datetime + 1 hour" +"%Y-%m-%dT%H:%M:%S")
      
      ./run_gpat_sensitivity.py --lat_bounds "47.0,48.0" --lon_bounds " -33.0,-32.0" --alt_bounds "12000,13000" --t0_sim "$datetime" --t0_fl "$t0_fl" \
   ...
   for $datetime in "${datetimes[@]}"; do
      ...
      ./run_gpat_sensitivity.py --lat_bounds "37.0,38.0" --lon_bounds " -97.0,-96.0" --alt_bounds "11000,12000" --t0_sim $datetime --t0_fl "$t0_fl" \
   ...
   for $datetime in "${datetimes[@]}"; do
      ...
      ./run_gpat_sensitivity.py --lat_bounds "42.0,43.0" --lon_bounds "7.0,8.0" --alt_bounds "9000,10000" --t0_sim $datetime --t0_fl "$t0_fl" \
   ...
   for $datetime in "${datetimes[@]}"; do
      ...
      ./run_gpat_sensitivity.py --lat_bounds "22.0,23.0" --lon_bounds "102.0,103.0" --alt_bounds "10000,11000" --t0_sim $datetime --t0_fl "$t0_fl" \
   ...
   for $datetime in "${datetimes[@]}"; do
      ...
      ./run_gpat_sensitivity.py --lat_bounds " -28.0,-27.0" --lon_bounds " -68.0,-67.0" --alt_bounds "13000,14000" --t0_sim $datetime --t0_fl "$t0_fl" \
