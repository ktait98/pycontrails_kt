...
from pycontrails.models.gpat.plume_to_grid_hres import plume_to_grid_hres
...
class FlParams:
    ...
    rt_fl: pd.Timedelta = field(default_factory=lambda: pd.Timedelta(minutes=60))  # flight run time
    ts_fl: pd.Timedelta = field(default_factory=lambda: pd.Timedelta(minutes=2))  # flight time step
...
class PlumeParams:
    ...
    vres_pl: float = 100  # vertical resolution of the plume [m]
...
class SimParams:
    ...
    hres_sim: float = 0.05  # horizontal resolution [deg]
    vres_sim: float = 1000  # vertical resolution [m]
...
class GPAT_HRES(Model):
    """Gridded Plume Analysis Tool (GPAT).

    Simulate aircraft trajectories, estimate aircraft performance, fuel burn and emissions. Then
    aggregates emissions to an Eulerian grid for investigation into how plume concentrations evolve over time.
    ...
    name = "GPAT"
    long_name = "Gridded Plume Analysis Tool (HRES)"
    # default_params = (FlParams, PlumeParams, SimParams)

    def __init__(self, fl_params: FlParams, plume_params: PlumeParams, sim_params: SimParams):
        super().__init__()
        ...
        self.alts_pl = np.arange(
            sim_params.alt_bounds[0],
            sim_params.alt_bounds[1] + plume_params.vres_pl / 2,
            plume_params.vres_pl,
        )
        self.lats = np.arange(
            sim_params.lat_bounds[0],
    ...
    def eval(self):
        ...
        # Aggregate plumes to an Eulerian grid
        start_wall_time = time.time()
        start_process_time = time.process_time()
    ...
    def gen_met(self) -> MetDataset:
        ...
        met = xr.Dataset(
            data_vars={
                "eastward_wind": (
                    ...
                    da.full(
                        (len(self.lats), len(self.lons), len(self.alts), len(self.times)),
                        sim_params.eastward_wind,
                    ),
                ),
                "northward_wind": (
                    ("latitude", "longitude", "level", "time"),
                ...
                "air_temperature": (
                    ("latitude", "longitude", "level", "time"),
                    da.zeros((len(self.lats), len(self.lons), len(self.alts), len(self.times))),
                ),
            },
            coords={
                "longitude": self.lons,
        ...
        air_temperature = (
            ...
            .sel(month=month - 1)
            .interp(longitude=self.lons, latitude=self.lats, level=self.levels, method="linear")
            .broadcast_like(met.data)
        )
    ...
    def gen_bg_chem(self) -> xr.Dataset:
        ...
        bg_chem = xr.open_dataset(self.inputs_glob + "species.nc", engine="netcdf4").sel(
            month=month - 1
        )
    ...
    def emissions(self) -> list[Flight]:
        ...
        for i, _fli in enumerate(fl):
            ...
            for species, ei in eis.items():
                fl[i][species] = ei * fl[i]["fuel_flow"] * plume_params.dt_integration.seconds
    ...
    def sim_plumes(self) -> list[pd.DataFrame]:
        ...
        filtered_plume_params_dict = {
            key: value
            for key, value in plume_params_dict.items()
            if key not in {"hres_pl", "vres_pl", "n_slices"}
        }
    ...
    def plume_to_grid(self) -> MetDataset:
        ...
        emi = xr.DataArray(
            np.zeros((len(self.lons), len(self.lats), len(self.alts), len(self.times), 9)),
            ...
            for t, time in enumerate(pl["time"].unique()):
                ...
                for p, property in enumerate(emi["emi_species"].values):
                    if property in self.sim_params.species_in:
                        ...
                        plume_property_data = emi.sel(
                            time=time,
                            emi_species=property)
                                            
                        plume_property_data = plume_to_grid_hres(
                            time=time,
                            plumes_t=plume_time_data,
                            var_name=property,
                            main_grid=plume_property_data,
                            grid_res=(sim_params.hres_sim, sim_params.vres_sim),
                        )
                        ...
                        density = plume_property_data / (
                            plume_params.vres_pl
                            * units.latitude_distance_to_m(plume_params.hres_pl)
                            * units.longitude_distance_to_m(
                                plume_params.hres_pl, (self.lats[0] + self.lats[-1]) / 2
                            )
                        )

                        plume = (density / 1e03) * NA / mm[p]
                        ...
                        plume = plume.reindex(
                            latitude=emi.latitude, longitude=emi.longitude, method="nearest"
                        )
                        ...
                        if time < pl["time"].unique()[-1]:
                            ...
                            emi.loc[:, :, :, time_slice, property] = plume_property_data
    ...
    def init_boxm_ds(self):
        ...
        self.boxm_ds["J"] = (
            ["time", "level", "longitude", "latitude", "photol_params"],
            da.zeros(
                (
                    self.boxm_ds.sizes["time"],
        ...
        self.boxm_ds["DJ"] = (
            ["time", "level", "longitude", "latitude", "photol_coeffs"],
            da.zeros(
                (
                    self.boxm_ds.sizes["time"],
                    self.boxm_ds.sizes["level"],
        ...
        self.boxm_ds["RC"] = (
            ["time", "level", "longitude", "latitude", "therm_coeffs"],
            da.zeros(
                (
                    self.boxm_ds.sizes["time"],
                    self.boxm_ds.sizes["level"],
    ...
    def stack(self):
        ...
        self.boxm_ds_stacked = self.boxm_ds.stack({"cell": ["level", "longitude", "latitude"]})
