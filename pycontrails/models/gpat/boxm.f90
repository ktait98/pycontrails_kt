...
CONTAINS 
    ...
    SUBROUTINE GET_PRE_INT_ATTS
        ...
        IERR = NF90_GET_ATT(NCID, NF90_GLOBAL, 'species_out', SPECIES_OUT)
    ...
    SUBROUTINE WRITE(DTS)
        ...
        ! Assuming species_out is already defined and allocated
        INTEGER :: I, SPECIES_INDEX

        ! Loop through the species_out array and write each species individually
        DO I = 1, SIZE(SPECIES_OUT)
            SPECIES_INDEX = SPECIES_OUT(I)
            IERR = NF90_PUT_VAR(NCID, VARID_Y, Y_PPB(:, SPECIES_INDEX), START=[1, SPECIES_INDEX, TS], COUNT=[NCELL, 1, 1])
            CALL CHECK(IERR, 'Y WRITE FAILED FOR SPECIES ' // TRIM(ADJUSTL(STRING(SPECIES_INDEX))))
        END DO      
