...
class GPATPostProcessor:
    ...
    def calc_NOPR(self, chem_ds):
        ...
        return NO * (kNO_HO2 * HO2 + kNO_CH3O2 * CH3O2) \
    ...
    def calc_NOy(self, chem_ds):
        ...
        return chem_ds["Y"].sel(species_out="NO") + chem_ds["Y"].sel(species_out="NO2") + \
    ...
    def calc_NOz(self, chem_ds):
        ...
        return chem_ds["Y"].sel(species_out="HNO3") + chem_ds["Y"].sel(species_out="PAN") + \
    ...
    @staticmethod
    def plot_scatter_plot(ax, x, y, title, xlabel, ylabel, xticks=None, 
                          yticks=None, grid=True):
    ...
    @staticmethod
    def plot_line_plot(ax, x, y, title, xlabel, ylabel, label=None, 
                       xticks=None, yticks=None, 
                       color='blue', grid=True, show_legend=True):
    ...
    @staticmethod
    def plot_spatial_heatmap(ax, data, lon, lat, value, title, xlabel, 
                             ylabel, xticks=None, 
                             yticks=None, grid=True):
        ...
        heatmap = ax.pcolormesh(data[lon], data[lat], data[value], cmap='viridis', 
    ...
    def plot_heatmap(params, fl_df, pl_df, chem_ds, **plot_params):
        ...
        if plot_params['da'] == "emi":
            heatmap_data = (
                chem_ds[plot_params['da']].sel(emi_species=plot_params['property'], 
                                               time=chem_ds["time"].values[plot_params["ts"]])
                .transpose("latitude", "longitude")
            )
        if plot_params['da'] == "Y":
            heatmap_data = (
                chem_ds[plot_params['da']].sel(species_out=plot_params['property'], 
                                               time=chem_ds["time"].values[plot_params["ts"]])
                .transpose("latitude", "longitude")
            )
        ...
        if plot_params['plot_pl']:
            ...
            for i, (_index, row) in enumerate(pl_df_time.iterrows()):
                ...
                dlon = units.m_to_longitude_distance(
                    width * np.sin(np.radians(heading)) * 0.5, lat
                    )
                ...
                patch = patches.Polygon(np.array(polygon.exterior.coords), closed=True, 
                                        color='blue', facecolor='none', alpha=0.1)
                ax1.add_patch(patch)
        ...
        for ts, time in enumerate(pl_df["time"].unique()[:-1]):
            ...
            if (grid_concs == 0).all():
                ...
                grid_mass = grid_concs_over_zero \
                    ...
                    * units.longitude_distance_to_m(params.loc["hres_sim"], 
                                                    (params.loc["lat_bounds"][0] + 
                                                     params.loc["lat_bounds"][1]) / 2) \
