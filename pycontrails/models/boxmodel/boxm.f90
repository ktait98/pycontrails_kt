MODULE BOXM
    ...
    CHARACTER(10) :: DATE
    CHARACTER(9) :: TIME
    DOUBLE PRECISION, ALLOCATABLE :: LEVEL(:), LON(:), LAT(:), TEMP(:), PRESSURE(:)
...
CONTAINS 
    ...
    SUBROUTINE CHECK(IERR, MSG)
        ...
        IF (IERR /= NF90_NOERR) THEN
            PRINT *, 'ERROR: CANNOT OPEN MET FILE'
            STOP
        END IF
    END SUBROUTINE CHECK

    ! OPEN MET, BG_CHEM AND EMI NC FILES
    SUBROUTINE OPEN_NC
        ! OPEN MET NC
        IERR = NF90_OPEN('boxm_input.nc', NF90_NOWRITE, NCID_MET)
        CALL CHECK(IERR, 'ERROR: CANNOT OPEN MET FILE')

        ! GET DIMIDS AND LENGTHS
        IERR = NF90_INQ_DIMID(NCID_MET, 'time', DIMID_TIME)
        CALL CHECK(IERR, 'ERROR: CANNOT GET TIME DIMID')
        IERR = NF90_INQUIRE_DIMENSION(NCID_MET, DIMID_TIME, 'time', NTS)
        CALL CHECK(IERR, 'ERROR: CANNOT GET TIME LENGTH')
        IERR = NF90_INQ_DIMID(NCID_MET, 'cell', DIMID_CELL)
        CALL CHECK(IERR, 'ERROR: CANNOT GET CELL DIMID')
        IERR = NF90_INQUIRE_DIMENSION(NCID_MET, DIMID_CELL, 'cell', NCELL)
        CALL CHECK(IERR, 'ERROR: CANNOT GET CELL LENGTH')

        ! GET VARIDS
        IERR = NF90_INQ_VARID(NCID_MET, 'air_temperature', VARID_TEMP)
        CALL CHECK(IERR, 'ERROR: CANNOT GET TEMP VARID')
        IERR = NF90_INQ_VARID(NCID_MET, 'specific_humidity', VARID_SPEC_HUM)
        CALL CHECK(IERR, 'ERROR: CANNOT GET SPEC_HUM VARID')
        IERR = NF90_INQ_VARID(NCID_MET, 'relative_humidity', VARID_REL_HUM)
        CALL CHECK(IERR, 'ERROR: CANNOT GET REL_HUM VARID')
        IERR = NF90_INQ_VARID(NCID_MET, 'eastward_wind', VARID_EAST_WIND)
        CALL CHECK(IERR, 'ERROR: CANNOT GET EAST_WIND VARID')
        IERR = NF90_INQ_VARID(NCID_MET, 'northward_wind', VARID_NORTH_WIND)
        CALL CHECK(IERR, 'ERROR: CANNOT GET NORTH_WIND VARID')
        IERR = NF90_INQ_VARID(NCID_MET, 'lagrangian_tendency_of_air_pressure', VARID_VERT_V)
        CALL CHECK(IERR, 'ERROR: CANNOT GET VERT_V VARID')
        IERR = NF90_INQ_VARID(NCID_MET, 'time', VARID_TIME)
        IERR = NF90_INQ_VARID(NCID_MET, 'air_pressure', VARID_PRESSURE)
        CALL CHECK(IERR, 'ERROR: CANNOT GET PRESSURE VARID')
        IERR = NF90_INQ_VARID(NCID_MET, 'altitude', VARID_ALT)
        CALL CHECK(IERR, 'ERROR: CANNOT GET ALT VARID')
        IERR = NF90_INQ_VARID(NCID_MET, 'M', VARID_M)
        CALL CHECK(IERR, 'ERROR: CANNOT GET M VARID')
        IERR = NF90_INQ_VARID(NCID_MET, 'H2O', VARID_H2O)
        CALL CHECK(IERR, 'ERROR: CANNOT GET H2O VARID')
        IERR = NF90_INQ_VARID(NCID_MET, 'O2', VARID_O2)
        CALL CHECK(IERR, 'ERROR: CANNOT GET O2 VARID')
        IERR = NF90_INQ_VARID(NCID_MET, 'N2', VARID_N2)
        CALL CHECK(IERR, 'ERROR: CANNOT GET N2 VARID')
        IERR = NF90_INQ_VARID(NCID_MET, 'sza', VARID_SZA)
        CALL CHECK(IERR, 'ERROR: CANNOT GET SZA VARID')
        IERR = NF90_INQ_VARID(NCID_MET, 'level', VARID_LEVEL)
        CALL CHECK(IERR, 'ERROR: CANNOT GET LEVEL VARID')
        IERR = NF90_INQ_VARID(NCID_MET, 'longitude', VARID_LON)
        CALL CHECK(IERR, 'ERROR: CANNOT GET LON VARID')
        IERR = NF90_INQ_VARID(NCID_MET, 'latitude', VARID_LAT)
        CALL CHECK(IERR, 'ERROR: CANNOT GET LAT VARID')
        
        ! OPEN BG_CHEM NC
        IERR = NF90_OPEN('bg_chem.nc', NF90_NOWRITE, NCID_BG_CHEM)
        CALL CHECK(IERR, 'ERROR: CANNOT OPEN BG_CHEM FILE')

        ! GET VARIDS

        IERR = NF90_OPEN('emi.nc', NF90_NOWRITE, NCID_EMI)
        CALL CHECK(IERR, 'ERROR: CANNOT OPEN EMI FILE')
    ...
    SUBROUTINE INIT_VARS()
        ...
        IF (.NOT. ALLOCATED(TEMP)) THEN
            ...
            TEMP(:) = 0.0
        END IF
        IF (.NOT. ALLOCATED(SPEC_HUM)) THEN
            ALLOCATE(SPEC_HUM(NCELL))
            SPEC_HUM(:) = 0.0
        END IF
        IF (.NOT. ALLOCATED(REL_HUM)) THEN
            ALLOCATE(REL_HUM(NCELL))
            REL_HUM(:) = 0.0
        END IF      
    ...
    SUBROUTINE READ(NCELL)
        ...
        DO CELL = 1,NCELL
            ...
            TEMP(CELL), SPEC_HUM(CELL), REL_HUM(CELL), U_WIND, V_WIND, VERT_V, &
             M(CELL), H2O(CELL), O2(CELL), N2(CELL), SZA(CELL)
