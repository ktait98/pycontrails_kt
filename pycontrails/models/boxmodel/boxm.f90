MODULE BOXM
    ...
    INTEGER :: IOSTAT
    REAL :: PI, TIME1, TSTORE

    INTEGER :: NTS, NCELL, NS, NEMI
    INTEGER :: NS, NTC, NPC, NPP, NFL, NEMI
    INTEGER :: S, CELL, TS

    INTEGER, PRIVATE :: NCID_IN, DIMID_TIME, DIMID_CELL, DIMID_NS, DIMID_NEMI
    INTEGER, PRIVATE :: DIMID_NPP, DIMID_NPC, DIMID_NTC, DIMID_NFL
...
CONTAINS 
    ...
    SUBROUTINE OPEN_NC
        IMPLICIT NONE
        ! OPEN BOXM INPUT NC
        IERR = NF90_OPEN('boxm_input.nc', NF90_NOWRITE, NCID_IN)
        CALL CHECK(IERR, 'ERROR: CANNOT OPEN BOXM_INPUT.NC')
    END SUBROUTINE OPEN_NC

    SUBROUTINE GET_DIMS
        IMPLICIT NONE
        ! GET DIMIDS AND LENGTHS
        IERR = NF90_INQ_DIMID(NCID_IN, 'time', DIMID_TIME)
        CALL CHECK(IERR, 'ERROR: CANNOT GET TIME DIMID')
        IERR = NF90_INQUIRE_DIMENSION(NCID_IN, DIMID_TIME, 'time', LEN=NTS)
        CALL CHECK(IERR, 'ERROR: CANNOT GET TIME LENGTH')
        IERR = NF90_INQ_DIMID(NCID_IN, 'cell', DIMID_CELL)
        CALL CHECK(IERR, 'ERROR: CANNOT GET CELL DIMID')
        IERR = NF90_INQUIRE_DIMENSION(NCID_IN, DIMID_CELL, 'cell', LEN=NCELL)
        CALL CHECK(IERR, 'ERROR: CANNOT GET CELL LENGTH')
        IERR = NF90_INQ_DIMID(NCID_IN, 'species', DIMID_NS)
        CALL CHECK(IERR, 'ERROR: CANNOT GET NS DIMID')
        IERR = NF90_INQUIRE_DIMENSION(NCID_IN, DIMID_NS, 'species', LEN=NS)
        CALL CHECK(IERR, 'ERROR: CANNOT GET NS LENGTH')
        IERR = NF90_INQ_DIMID(NCID_IN, 'emi_species', DIMID_NEMI)
        CALL CHECK(IERR, 'ERROR: CANNOT GET NEMI DIMID')
        IERR = NF90_INQUIRE_DIMENSION(NCID_IN, DIMID_NEMI, 'emi_species', LEN=NEMI)
    ...
    SUBROUTINE GET_PRE_INT_VARS
        ...
        IERR = NF90_GET_VAR(NCID_IN, VARID_TIME, TIME)
        CALL CHECK(IERR, 'ERROR: CANNOT GET TIME ARRAY')
        
        IERR = NF90_GET_VAR(NCID_IN, VARID_LEVEL, LEVEL)
        CALL CHECK(IERR, 'ERROR: CANNOT GET LEVEL ARRAY')
        
        IERR = NF90_GET_VAR(NCID_IN, VARID_LON, LON)
        CALL CHECK(IERR, 'ERROR: CANNOT GET LON ARRAY')
        
        IERR = NF90_GET_VAR(NCID_IN, VARID_LAT, LAT)
        CALL CHECK(IERR, 'ERROR: CANNOT GET LAT ARRAY')
        
        IERR = NF90_GET_VAR(NCID_IN, VARID_PRESSURE, PRESSURE)
        CALL CHECK(IERR, 'ERROR: CANNOT GET PRESSURE ARRAY')
        
        IERR = NF90_GET_VAR(NCID_IN, VARID_ALT, ALT)
        CALL CHECK(IERR, 'ERROR: CANNOT GET ALT ARRAY')
        
        IERR = NF90_GET_VAR(NCID_IN, VARID_SPECIES, SPECIES)
        CALL CHECK(IERR, 'ERROR: CANNOT GET SPECIES ARRAY')
        
        IERR = NF90_GET_VAR(NCID_IN, VARID_EMI_SPECIES, EMI_SPECIES)
        CALL CHECK(IERR, 'ERROR: CANNOT GET EMI_SPECIES ARRAY')
        
        IERR = NF90_GET_VAR(NCID_IN, VARID_BG_CHEM, BG_CHEM)
        CALL CHECK(IERR, 'ERROR: CANNOT GET BG_CHEM ARRAY')

        ! ASSIGN BG_CHEM TO FIRST TIMESTEP OF Y
        Y(:,:) = BG_CHEM(:,:) * M(:)/1E+09 ! CONVERT FROM PPB TO MOL/CM3 FOR KINETICS
    ...
    SUBROUTINE INIT_CHEM
        ...
        IERR = NF90_CREATE('chem.nc', NF90_CLOBBER, NCID_CHEM)
        CALL CHECK(IERR, 'ERROR: CANNOT CREATE OUTPUT FILE')
  
        ! DEFINE DIMS
        IERR = NF90_DEF_DIM(NCID_CHEM, 'time', NTS, DIMID_TIME)
        CALL CHECK(IERR, 'ERROR: CANNOT DEFINE TIME DIM')

        IERR = NF90_DEF_DIM(NCID_CHEM, 'cell', NCELL, DIMID_CELL)
        CALL CHECK(IERR, 'ERROR: CANNOT DEFINE CELL DIM')

        IERR = NF90_DEF_DIM(NCID_CHEM, 'species', NS, DIMID_NS)
        CALL CHECK(IERR, 'ERROR: CANNOT DEFINE NS DIM')

        IERR = NF90_DEF_DIM(NCID_CHEM, 'photol_params', NPP, DIMID_NPP)
        CALL CHECK(IERR, 'ERROR: CANNOT DEFINE NPP DIM')
        
        IERR = NF90_DEF_DIM(NCID_CHEM, 'photol_coeffs', NPC, DIMID_NPC)
        CALL CHECK(IERR, 'ERROR: CANNOT DEFINE NPC DIM')

        IERR = NF90_DEF_DIM(NCID_CHEM, 'therm_coeffs', NTC, DIMID_NTC)
        CALL CHECK(IERR, 'ERROR: CANNOT DEFINE NTC DIM')

        IERR = NF90_DEF_DIM(NCID_CHEM, 'emissions', NEMI, DIMID_NEMI)
        CALL CHECK(IERR, 'ERROR: CANNOT DEFINE NEMI DIM')

        ! DEFINE VARS
        IERR = NF90_DEF_VAR(NCID_CHEM, 'level', NF90_DOUBLE, [DIMID_CELL], VARID_LEVEL)
        CALL CHECK(IERR, 'ERROR: CANNOT DEFINE LEVEL VAR')

        IERR = NF90_DEF_VAR(NCID_CHEM, 'longitude', NF90_DOUBLE, [DIMID_CELL], VARID_LON)
        CALL CHECK(IERR, 'ERROR: CANNOT DEFINE LON VAR')

        IERR = NF90_DEF_VAR(NCID_CHEM, 'latitude', NF90_DOUBLE, [DIMID_CELL], VARID_LAT)
        CALL CHECK(IERR, 'ERROR: CANNOT DEFINE LAT VAR')

        IERR = NF90_DEF_VAR(NCID_CHEM, 'Y', NF90_DOUBLE, [DIMID_TIME, DIMID_CELL, DIMID_NS], VARID_Y)
        CALL CHECK(IERR, 'ERROR: CANNOT DEFINE Y VAR')

        IERR = NF90_DEF_VAR(NCID_CHEM, 'SZA', NF90_DOUBLE, [DIMID_TIME, DIMID_CELL], VARID_SZA)
        CALL CHECK(IERR, 'ERROR: CANNOT DEFINE SZA VAR')

        IERR = NF90_DEF_VAR(NCID_CHEM, 'J', NF90_DOUBLE, [DIMID_TIME, DIMID_CELL, DIMID_NPP], VARID_J)
        CALL CHECK(IERR, 'ERROR: CANNOT DEFINE J VAR') 

        IERR = NF90_DEF_VAR(NCID_CHEM, 'DJ', NF90_DOUBLE, [DIMID_TIME, DIMID_CELL, DIMID_NPC], VARID_DJ)
        CALL CHECK(IERR, 'ERROR: CANNOT DEFINE DJ VAR')

        IERR = NF90_DEF_VAR(NCID_CHEM, 'RC', NF90_DOUBLE, [DIMID_TIME, DIMID_CELL, DIMID_NTC], VARID_RC)
        CALL CHECK(IERR, 'ERROR: CANNOT DEFINE RC VAR')

        IERR = NF90_DEF_VAR(NCID_CHEM, 'EMI', NF90_DOUBLE, [DIMID_TIME, DIMID_CELL, DIMID_NEMI], VARID_EMI)
        CALL CHECK(IERR, 'ERROR: CANNOT DEFINE EMI VAR')

        ! END DEF
        IERR = NF90_ENDDEF(NCID_CHEM)
        CALL CHECK(IERR, 'ERROR: CANNOT END DEF VAR')
    ...
    SUBROUTINE GET_INT_VARS(TS)
        ...
        IERR = NF90_GET_VAR(NCID_IN, VARID_TEMP, TEMP, START, COUNT)
        CALL CHECK(IERR, 'ERROR: CANNOT GET TEMP ARRAY')
        
        IERR = NF90_GET_VAR(NCID_IN, VARID_M, M, START, COUNT)
        CALL CHECK(IERR, 'ERROR: CANNOT GET M ARRAY')
        
        IERR = NF90_GET_VAR(NCID_IN, VARID_H2O, H2O, START, COUNT)
        CALL CHECK(IERR, 'ERROR: CANNOT GET H2O ARRAY')
        
        IERR = NF90_GET_VAR(NCID_IN, VARID_O2, O2, START, COUNT) 
        CALL CHECK(IERR, 'ERROR: CANNOT GET O2 ARRAY')
        
        IERR = NF90_GET_VAR(NCID_IN, VARID_N2, N2, START, COUNT)
        CALL CHECK(IERR, 'ERROR: CANNOT GET N2 ARRAY')
        
        IERR = NF90_GET_VAR(NCID_IN, VARID_SZA, SZA, START, COUNT)
        CALL CHECK(IERR, 'ERROR: CANNOT GET SZA ARRAY')
        
        ! SET PREVIOUS EMI TO EMIP
        EMIP(:,:) = EMI(:,:)

        IERR = NF90_GET_VAR(NCID_IN, VARID_EMI, EMI, (/TS, 1, 1/), (/1, NCELL, NEMI/))
