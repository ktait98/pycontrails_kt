...
CONTAINS
    ...
    SUBROUTINE INIT(NCELL, NTS)
        ...
        ! OPEN MET CSV FILE
        OPEN(UNIT=9, FILE='met_df.csv', STATUS='OLD', ACTION='READ')
        READ(9, '(A)') MET_HEADER

        ! OPEN BG_CHEM CSV FILE
        OPEN(UNIT=10, FILE='bg_chem_df.csv', STATUS='OLD', ACTION='READ')
        READ(10, '(A)') CHEM_HEADER
        ...
        ! DEFINE VARS
        IERR = NF90_DEF_VAR(NCID, 'level', NF90_DOUBLE, [DIMID_CELL], VARID_LEVEL)
        IF (IERR /= NF90_NOERR) THEN
            PRINT *, 'ERROR: CANNOT DEFINE LEVEL VAR'
            STOP
        END IF
        IERR = NF90_DEF_VAR(NCID, 'longitude', NF90_DOUBLE, [DIMID_CELL], VARID_LON)
        IF (IERR /= NF90_NOERR) THEN
            PRINT *, 'ERROR: CANNOT DEFINE LON VAR'
            STOP
        END IF
        IERR = NF90_DEF_VAR(NCID, 'latitude', NF90_DOUBLE, [DIMID_CELL], VARID_LAT)
        IF (IERR /= NF90_NOERR) THEN
